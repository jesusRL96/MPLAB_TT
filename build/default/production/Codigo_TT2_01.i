...
int angulo = 0, alto_bajo = 0, parpadeo = 0, alerta = 0, auxiliar = 0;
void __attribute__((picinterrupt(("high_priority")))) timer2(void){
    TMR2IF=0;
    ...
    while(1){
        ...
        if(creditos == 0) Menu1();
            ...
            if (PORTAbits.RA1) {
                ...
                if(creditos >= 2) {
                    motor_temp = MotorPasos(motorBarritas, 203, 1, 1, 1, 2);
        ...
        motor_temp = MotorPasos_sensor(motorRecepcion, 28);
        ...
        if (PORTCbits.RC3 && (altura > 88)) {
            aceptado = ((peso > 287) && (peso < 304)) && ((altura > 90) && (altura < 194));
            angulo = 45;
            pet = 0;
        }
        else if(PORTCbits.RC3 && (altura >= 76) && (altura < 88)) {
            aceptado = ((peso > 287) && (peso < 304)) && ((altura > 30) && (altura < 80));
            if (aceptado) angulo = 0;
            pet = 1;
        }
        else {
            aceptado = ((peso > 287) && (peso < 304)) && ((altura > 30) && (altura < 80));
...
int MotorPasos(int motor_p,int pasos,int sentido,int tipo, int num, int retardo) {
    ...
    for(int i = 0; i < pasos;i++){
        ...
        if(retardo == 1) {
            ...
            _delay((unsigned long)((45)*(8000000/4000.0)));
        }
    }
    return n;
}
